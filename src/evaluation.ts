...
export async function runSelectedEvaluation(
  ...
  for (const imageName of selectedImageNames) {
    ...
    try {
      ...
      // --- *** THIS IS THE FIX *** ---
      // Handle the "no_shapes" case for averaging
      if (gtShapes.length === 0 && detectionResult.shapes.length === 0) {
        totalPrecision += 1.0;
        totalRecall += 1.0;
        totalF1 += 1.0;
      } else {
...
export async function runEvaluation(
  ...
  for (const imageName of imageNames) {
    ...
    try {
      ...
      // --- *** THIS IS THE FIX (Applied in both functions) *** ---
      // Handle the "no_shapes" case for averaging
      if (gtShapes.length === 0 && detectionResult.shapes.length === 0) {
        totalPrecision += 1.0;
        totalRecall += 1.0;
    ...
    } catch (error) { // --- *** SYNTAX FIX IS HERE *** ---
      console.error(`Error testing ${imageName}:`, error);
      testResults.push({
        imageName,
        detectionResult: {
...
/**
 * --- MODIFIED ---
 * Added gtShapes to the function signature
 */
function calculateScore(
  evaluation: EvaluationMetrics,
  detection: DetectionResult,
  gtShapes: any[] // Added this parameter
): {
  passed: boolean;
  feedback: string[];
  score: number;
  ...
  // --- *** THIS IS THE FIX *** ---
  // Special case for "no_shapes.png"
  // If 0 shapes are expected and 0 shapes are detected, it's a 100% pass.
  if (gtShapes.length === 0 && detection.shapes.length === 0) {
    return {
      passed: true,
      feedback: [
        `✓ Correctly detected 0 shapes (F1: 1.000)`,
        `✓ localization (IoU: N/A)`,
        `✓ center accuracy (N/A)`,
        `✓ area calculation (N/A)`,
        `✓ performance (${detection.processingTime.toFixed(0)}ms)`,
      ],
      score: 100, // Full points for this test
    };
  }
  // --- END FIX ---
